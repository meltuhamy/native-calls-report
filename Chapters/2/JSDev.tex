...
\lstset{language=C,caption={An example of using the module pattern},label=code_module_pattern_js}
\begin{code}
...
\lstset{language=C,caption={JavaScript `classes' using the module pattern},label=code_js_module_classes}
\begin{code}
...
\subsection{File Organisation} % (fold)
...
\subsubsection{Asynchronous Module Definition (AMD)} % (fold)
...
\lstset{language=C,caption={MyClass AMD Module},label=code_myclass_amd_module}
\begin{code}
// MyClass.js
...
% subsubsection asynchronous_module_definition_ (end)

\subsubsection{CommonJS Modules} % (fold)
...
\lstset{language=C,caption={MyClass CommonJS Module},label=code_commonjs_modules}
\begin{code}
// MyClass.js
var private = 23;
...
% subsubsection commonjs_modules (end)

% subsection js_file_organisation (end)
